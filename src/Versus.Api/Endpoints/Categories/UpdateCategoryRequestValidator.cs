...
public class UpdateCategoryRequestValidator : AbstractValidator<UpdateCategoryRequest>
{
    ...
    public UpdateCategoryRequestValidator(VersusDbContext dbContext)
    {
    ...
    private async Task<bool> UniqueName(UpdateCategoryRequest request, string name,
        ValidationContext<UpdateCategoryRequest> context, CancellationToken cancellationToken)
    {
        var categoryId = (Guid)context.RootContextData["CategoryId"];
        return !await _dbContext.Categories
            .Where(x => x.Id != categoryId && x.Name.ToLower() == name.ToLower())
